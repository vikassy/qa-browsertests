...
Then(/^I can select enable enhanced editing toolbar$/) do
  on(PreferencesEditingPage).enhanced_editing_toolbar_check_element.when_present.should exist
end

Then(/^I can select enable wizards for inserting links, tables as well as the search and replace function$/) do
  on(PreferencesEditingPage).wizard_check_element.when_present.should exist
end
